...
/**
 * Custom health indicator that verifies database connectivity by executing
 * a simple JDBC query (SELECT 1). This ensures the /actuator/health endpoint
 * reports DOWN when the database is unreachable, rather than relying solely
 * on HikariCP pool status which may mask connection failures.
 */
@Component
