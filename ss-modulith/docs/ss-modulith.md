...
- **Subject ordering drives selection**: `QuizBundleService` picks the first `QUIZZES_PER_CLASS` active subjects per class by `Subject.displayOrder` (then id) — not DB insert order. Subjects carry an admin-editable `displayOrder`; `SubjectDisplayOrderInitializer` assigns legacy subjects curated defaults (Math=1, EVS=2, English=3, Hindi=4, General Knowledge=5, others=10) idempotently at startup (only where displayOrder is still 0). Class-grade subject listings (`/api/v1/subjects/class-grade/{id}`) return this order too.
